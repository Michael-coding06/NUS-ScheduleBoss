const TasksSection = ({
  ...
  const handleDeleteTask = (index, taskName) => {
    ...
    setHighlightedSpans(highlightedSpans.filter(span => span.name !== taskName));
  };
          ...
          <div key={index} className="task-card">
            ...
            <div className="mod-action">
              <button className="icon" onClick={(e) => {
                  e.stopPropagation();
                  setTasks(tasks.filter((_, i) => i !== index));
                  setSelectedSpans(selectedSpans.filter(span => span.name !== task.name));
                  setHighlightedSpans(highlightedSpans.filter(span => span.name !== task.name));
                  // handleDeleteTask(index, task.name);
                }}
              >
                🗑️
              </button>
              <button className="icon" onClick={(e) => {
                  e.stopPropagation();
                  handleToggleTaskVisibility(task.name);
                }}
              >
                🙈
              </button>
